...
def flux_model(param):
    ...
    flux_array = np.array([flux1,]*(kelly_repeat))
    dt = np.array([abs(np.diff(time1)),]*kelly_repeat)
    epsilon = np.random.normal(0,1,size=np.shape(dt))
    dX1 = flux1[1:]
    change = dX(tau,sigma_tot,dt,b,epsilon,dX1)
...
for i in range(runs):
    ...
    for i1 in range(runs1):
        ...
        for j in range(len(cont_days)-3):
            ...
            cont_flux = array([cont[:,1],]*len(data_comp[:,0])).transpose()
            data_comp[:,2] = sum(step*cont_flux*transfer_array,axis=0)
