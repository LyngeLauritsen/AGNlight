...
for i in range(runs):
    ...
    for i1 in range(runs1):
        ...
        if try1 == 0:
            ...
            change = rfft(colour(np.random.normal(2.8,0.25,1)[0],len(cont[:,1])))
            weight = random.random()*0.005 #0.05
            cont_fft = np.exp((1 - weight)*np.log(cont_fft) + weight*np.log(change))
            #print try1
        if try1 == 1.1:
            '''Thermal component'''
            T_direction = (-1)**np.random.randint(2,size=1) #np.array([(-1)**np.random.randint(2,size=1)*random.random()*np.random.randint(5,size=1) for i in range(len(cont_days))]) # K
        ...
        if chi2 < chi1 \
           ...
           and (data_comp_z[:,2] >= 0.).all() == True: # \
            ...
            if try1 == 0 and random.random() < 0.01:
                try1 = 1.1
