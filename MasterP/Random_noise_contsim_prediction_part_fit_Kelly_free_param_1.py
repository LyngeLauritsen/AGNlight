...
def prob_K(param_K):
    ...
    return np.sum((data_comp_K[:,1] - N)**2/data_comp_K[:,3]**2)

param_H = [1.]
def prob_H(param_H):
    N = determine_constants(cont_days,mu_power_H,sigma_power_H,lag_thermal,width_thermal,param_H[0],BB_H,A_T,T,data_comp_H)
    return np.sum((data_comp_H[:,1] - N)**2/data_comp_H[:,3]**2)

param_J = [1.]
def prob_J(param_J):
    N = determine_constants(cont_days,mu_power_J,sigma_power_J,lag_thermal,width_thermal,param_J[0],BB_J,A_T,T,data_comp_J)
    return np.sum((data_comp_J[:,1] - N)**2/data_comp_J[:,3]**2)

param_g = [1.]
def prob_g(param_g):
    N = determine_constants(cont_days,mu_power_g,sigma_power_g,lag_thermal,width_thermal,param_g[0],BB_g,A_T,T,data_comp_g)
    return np.sum((data_comp_g[:,1] - N)**2/data_comp_g[:,3]**2)

param_r = [1.]
def prob_r(param_r):
    N = determine_constants(cont_days,mu_power_r,sigma_power_r,lag_thermal,width_thermal,param_r[0],BB_r,A_T,T,data_comp_r)
    return np.sum((data_comp_r[:,1] - N)**2/data_comp_r[:,3]**2)
...
for i in range(runs):
    ...
    mu_power_K += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_H += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_J += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_g += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_r += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_i += (-1)**np.random.randint(2)*0.2*random.random()
    mu_power_z += (-1)**np.random.randint(2)*0.2*random.random()

    sigma_power_K += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_H += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_J += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_g += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_r += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_i += (-1)**np.random.randint(2)*0.02*random.random()
    sigma_power_z += (-1)**np.random.randint(2)*0.02*random.random()
    ...
    weight = random.random()/float(random.randint(10,50)) #0.05
    ...
    for i1 in range(runs1):
        ...
        if try1 == 0 and random.random() < 0.05:
            change = rfft(colour(np.random.normal(2.8,0.25,1)[0],len(cont[:,1]),np.nanmean(irfft(cont_fft)))) #change_size) #len(cont[:,1]))) rfft
            weight = random.random()/float(random.randint(50,5000)) #0.05
            cont_fft = np.exp((1 - weight)*np.log(cont_fft) + weight*np.log(change)) #origin2))
        if try1 == 0:
            ...
            weight = random.random()/float(random.randint(2,5000)) #0.05
            cont_fft = np.exp((1 - weight)*np.log(cont_fft) + weight*np.log(change)) #origin2))
            #print try1
        elif try1 == 1.1:
            '''Thermal component'''
        ...
        if 1 < chi2 < chi1 \
           ...
           and (data_comp_z[:,2] >= 0.).all() == True: # \
            ...
            if try1 == 3 and i1 > 1:
                ...
                try1 = 1.1
            elif (try1 == 1.1 or try1 == 2.1): # and random.random() < 0.1: # and random.random() < 0.2: # and random.random() < 0.3:
                try1 = 0
    ...
    plt.plot(x_list,transfer_power_K,color='g',label='K')
    plt.plot(x_list,transfer_power_H,color='r',label='H')
    plt.plot(x_list,transfer_power_J,color='b',label='J')
    plt.plot(x_list,transfer_power_g,color='k',label='g')
    plt.plot(x_list,transfer_power_r,color='brown',label='r')
    plt.plot(x_list,transfer_power_i,color='yellow',label='i')
    plt.plot(x_list,transfer_thermal,color='orange',label='Thermal')
    ...
    plt.plot([1,2,3,4,5,6],N_s_power,label='N_s_power')
    plt.plot([1,2,3,4,5,6],[mu_power_K,mu_power_H,mu_power_J,mu_power_g,\
                            mu_power_r,mu_power_i],label='Lag Power')
    plt.plot([1,2,3,4,5,6],[sigma_power_K,sigma_power_H,sigma_power_J,sigma_power_g,\
                            sigma_power_r,sigma_power_i],label='Sigma Power')
    plt.xticks([1,2,3,4,5,6],[K_cen[0],H_cen[0],J_cen[0],g_cen[0],r_cen[0],z_cen[0]]) #['K','H','J','g','r','i'])
    #K_cen[0],H_cen[0],J_cen[0],g_cen[0],r_cen[0],z_cen[0]]
    plt.legend()
    #plt.xlim([0,400])
