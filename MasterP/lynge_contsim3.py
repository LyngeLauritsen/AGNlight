...
for i in range(runs):
    ...
    for i1 in range(runs1):
        ...
        for j in range(1):
            ...
            colour_change = colour(-slopeaim,len(cont[:,1]))
            #colour_change = colour_change/np.std(colour_change)
            
            change = colour_change*10**(-20) #Finding the magnitude and direction of change
            #print powerlaw_psd_gaussian(2.5,10)
            #print 'HEY', np.nanmean(change), np.std(change)
            cont[:,1] = 10**(np.log10(cont[:,1])*0.5 + np.log10(change)*0.5) #*random.random()*900.) #/np.sqrt(2.) #Implementing change
            #print change
            #print cont[:,1]
            h = 0
