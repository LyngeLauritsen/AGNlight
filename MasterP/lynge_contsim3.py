...
runs1 = 40
days_before = 500. #How far to extend the lightcurve back (days)
...
#cont = np.loadtxt('CONTINUUM/NGC3783-continuum-slope-2-5-6')
day = cont_days[0]
...
data_comp[:,1] = data[:,1]
data_comp[:,2] = 0
#data_comp = np.loadtxt('CONTINUUM/NGC3783-data_comp-slope-2-5-6')
#print data[:,2]
...
for i in range(runs):
    ...
    for i1 in range(runs1):
        ...
        for j in range(len(cont_days)-3):
            ...
            if chi2 <= chi1 and slopechange < 0. and dd1_ddt1 < dd_ddt_allowed and dd2_ddt1 < dd_ddt_allowed \
               and dd3_ddt1 < dd_ddt_allowed: 
                ...
                print 1, slope, dd1_ddt1
                 ...
                 and dd2_ddt1 < dd_ddt_allowed and dd3_ddt1 < dd_ddt_allowed: 
                '''The second instance of acceptance'''
                chi1 = chi2
                slopeolder1 = slope
                print 2, slope

            elif (dd1_ddt1 - dd1_ddt) + (dd2_ddt1 - dd2_ddt) + (dd3_ddt1 - dd3_ddt) < 0 and random.random() < 0.005: 
            ...
            gc.collect()
    print P_v
    np.savetxt('CONTINUUM/NGC3783-continuum-slope-3-1',cont) #Updating the files
    np.savetxt('CONTINUUM/NGC3783-data_comp-slope-3-1',data_comp)
    plt.figure()
