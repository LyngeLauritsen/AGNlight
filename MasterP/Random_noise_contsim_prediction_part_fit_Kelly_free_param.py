...
data_g = np.loadtxt('NOVEMBER/NOV-NGC3783-g_short') #'NOVEMBER/Kelly-NGC3783K')
#error_g = np.loadtxt('NOVEMBER/NGC3783_NOISE_g.txt') #Load data

data_r = np.loadtxt('NOVEMBER/NOV-NGC3783-r_short') #'NOVEMBER/Kelly-NGC3783K')
#error_r = np.loadtxt('NOVEMBER/NGC3783_NOISE_r.txt') #Load data

data_i = np.loadtxt('NOVEMBER/NOV-NGC3783-i_short') #'NOVEMBER/Kelly-NGC3783K')
#error_i = np.loadtxt('NOVEMBER/NGC3783_NOISE_i.txt') #Load data

data_z = np.loadtxt('NOVEMBER/NOV-NGC3783-z_short') #'NOVEMBER/Kelly-NGC3783K')
...
for i in range(runs):
    ...
    for i1 in range(runs1):
        ...
        if try1 == 0 and random.random() < 0.2:
            change = rfft(colour(np.random.normal(2.8,0.25,1)[0],len(cont[:,1]),np.nanmean(irfft(cont_fft)))) #change_size) #len(cont[:,1]))) rfft
    ...
    plt.plot(x_list,transfer_power_K,color='k',label='g')
    plt.plot(x_list,transfer_power_H,color='brown',label='r')
    plt.plot(x_list,transfer_power_J,color='yellow',label='i')
    ...
    plt.xticks([1,2,3,4,5,6],['K','H','J','g','r','i'])
